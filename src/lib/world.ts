...
export function worldPrimer(): string {
  ...
  lines.push("\n[경지]");
  lines.push("삼류·이류·일류·절정·초절정·화경·현경·생사경. 각 경지 초입·완숙·극 3단계.");
  lines.push("절정 이상은 천하 2% 미만. 화경은 한 세대 십수 명. 현경은 한 시대 한둘. 생사경(生死境)은 인간 한계 너머 — 아직 누구도 닿지 못한 전설의 경지.");
  lines.push("현 시대는 무림 황금기 — 육천 중 둘(마천·은화천)이 이미 현경 극에 닿았고, 마천은 생사경의 벽을 두드린다는 풍문이 돈다.");
  lines.push("\n[정점] 육천(6, 현경, 둘은 현경 극) / 오황(5, 화경 완숙~극, 帝) / 팔왕(8, 화경 초입~완숙, 王) / 칠성(7, 초절정 극~화경 완숙). 전반적으로 한 단계씩 더 강해진 시대.");
  lines.push("\n[5대 긴장]");
